...
pub trait TyOld<'ast>: Debug {
    fn get_kind(&'ast self) -> &'ast TyKindOld<'ast>;

    fn is_unit(&'ast self) -> bool {
        matches!(self.get_kind(), TyKindOld::Tuple(&[]))
    }
...
pub enum TyKindOld<'ast> {
    ...
    Ref(&'ast dyn TyOld<'ast>, Mutability, &'ast dyn Lifetime<'ast>),

    RawPtr(&'ast dyn TyOld<'ast>, Mutability),
...
pub struct FunctionTy<'ast> {
    return_ty: Option<&'ast dyn TyOld<'ast>>,

    args: &'ast [&'ast dyn TyOld<'ast>],
}

#[cfg(feature = "driver-api")]
impl<'ast> FunctionTy<'ast> {
    #[must_use]
    pub fn new(return_ty: Option<&'ast dyn TyOld<'ast>>, args: &'ast [&'ast dyn TyOld<'ast>]) -> Self {
        Self { return_ty, args }
    }
}

impl<'ast> FunctionTy<'ast> {
    pub fn get_return_ty(&'ast self) -> Option<&'ast dyn TyOld<'ast>> {
        self.return_ty
    }

    pub fn get_args(&'ast self) -> &'ast [&'ast dyn TyOld<'ast>] {
        self.args
    }
}
