use marker_api::ast::{
    generic::{
        ...
        SynGenericParams, SynLifetimeArg, SynLifetimeClause, SynLifetimeParam, SynTraitBound, SynTyArg, SynTyClause,
        SynTyParam, SynTyParamBound, SynWhereClauseKind,
    },
    ConstValue, TraitRef,
};
        ...
        if let Some(main) = bounds.principal() {
            ...
            bounds
                ...
                .for_each(|binding| match binding.skip_binder().term.unpack() {
                    mid::ty::TermKind::Ty(ty) => generics.push(SemGenericArgKind::Binding(self.alloc(
                        SemBindingArg::new(self.to_item_id(binding.item_def_id()), self.to_sem_ty(ty)),
                    ))),
...
impl<'ast, 'tcx> MarkerConverterInner<'ast, 'tcx> {
    pub fn to_syn_generic_args_from_path(&self, rust_path: &rustc_hir::Path<'tcx>) -> SynGenericArgs<'ast> {
        self.to_syn_generic_args(rust_path.segments.last().and_then(|s| s.args))
    }

    #[must_use]
    pub fn to_syn_generic_args(&self, rustc_args: Option<&hir::GenericArgs<'tcx>>) -> SynGenericArgs<'ast> {
        ...
        let mut args: Vec<_> = rustc_args
            ...
            .filter_map(|rustc_arg| match rustc_arg {
                rustc_hir::GenericArg::Lifetime(rust_lt) => self
                    .to_lifetime(rust_lt)
                    .map(|lifetime| SynGenericArgKind::Lifetime(self.alloc(SynLifetimeArg::new(lifetime)))),
                rustc_hir::GenericArg::Type(r_ty) => {
                    Some(SynGenericArgKind::Ty(self.alloc(SynTyArg::new(self.to_syn_ty(r_ty)))))
                },
                rustc_hir::GenericArg::Const(arg) => Some(SynGenericArgKind::Const(self.alloc(SynConstArg::new(
    ...
    pub fn to_syn_generic_params(&self, rustc_generics: &hir::Generics<'tcx>) -> SynGenericParams<'ast> {
            ...
            .filter_map(|predicate| {
                match predicate {
                    hir::WherePredicate::BoundPredicate(ty_bound) => {
                        ...
                        let params =
                            SynGenericParams::new(self.to_syn_generic_param_kinds(ty_bound.bound_generic_params), &[]);
                        let ty = self.to_syn_ty(ty_bound.bounded_ty);
                        Some(SynWhereClauseKind::Ty(self.alloc({
                            SynTyClause::new(Some(params), ty, self.to_syn_ty_param_bound(predicate.bounds()))
                        })))
                    },
        ...
        SynGenericParams::new(self.to_syn_generic_param_kinds(rustc_generics.params), clauses)
    }

    fn to_syn_generic_param_kinds(&self, params: &[hir::GenericParam<'tcx>]) -> &'ast [SynGenericParamKind<'ast>] {
        if params.is_empty() {
            return &[];
        }
        ...
        let params: Vec<_> = params
            ...
            .filter_map(|rustc_param| {
                ...
                match rustc_param.kind {
                    ...
                    } => Some(SynGenericParamKind::Lifetime(self.alloc(SynLifetimeParam::new(
                        id,
                        name,
                        Some(span),
                    )))),
                    hir::GenericParamKind::Type { synthetic: false, .. } => Some(SynGenericParamKind::Ty(
                        self.alloc(SynTyParam::new(Some(span), name, id)),
                    )),
    ...
    #[must_use]
    pub fn to_syn_ty_param_bound(&self, bounds: &[hir::GenericBound<'tcx>]) -> &'ast [SynTyParamBound<'ast>] {
        if bounds.is_empty() {
            return &[];
        }
        ...
        let bounds: Vec<_> = bounds
            ...
            .filter_map(|bound| match bound {
                ...
                hir::GenericBound::LangItemTrait(lang_item, span, _, rustc_args) => Some(SynTyParamBound::TraitBound(
                    self.alloc(SynTraitBound::new(
                        false,
                        TraitRef::new(
                            self.to_item_id(
                                self.rustc_cx
