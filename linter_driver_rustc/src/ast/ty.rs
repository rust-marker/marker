...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, NumericKind> for rustc_ast::ast::IntTy {
    fn to_api(&self, _cx: &'ast RustcContext<'ast, 'tcx>) -> NumericKind {
        match self {
...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, NumericKind> for rustc_ast::ast::UintTy {
    fn to_api(&self, _cx: &'ast RustcContext<'ast, 'tcx>) -> NumericKind {
        match self {
...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, NumericKind> for rustc_ast::ast::FloatTy {
    fn to_api(&self, _cx: &'ast RustcContext<'ast, 'tcx>) -> NumericKind {
        match self {
...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, TyKind<'ast>> for rustc_hir::PrimTy {
    fn to_api(&self, cx: &'ast RustcContext<'ast, 'tcx>) -> TyKind<'ast> {
        match self {
            rustc_hir::PrimTy::Int(inner) => TyKind::Numeric(inner.to_api(cx)),
            rustc_hir::PrimTy::Uint(inner) => TyKind::Numeric(inner.to_api(cx)),
            rustc_hir::PrimTy::Float(inner) => TyKind::Numeric(inner.to_api(cx)),
...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, TyKind<'ast>> for rustc_hir::Path<'tcx> {
    fn to_api(&self, cx: &'ast RustcContext<'ast, 'tcx>) -> TyKind<'ast> {
        match self.res {
            rustc_hir::def::Res::Def(def_kind, def_id) => {
                ...
                match def_kind {
                    ...
                    | rustc_hir::def::DefKind::Struct => TyKind::Adt(def_id.to_api(cx)),
                    rustc_hir::def::DefKind::TyAlias => TyKind::TyAlias(def_id.to_api(cx)),
                    rustc_hir::def::DefKind::ForeignTy => TyKind::ForeignTy(def_id.to_api(cx)),
                    rustc_hir::def::DefKind::OpaqueTy => TyKind::ImplTrait(def_id.to_api(cx)),
            ...
            rustc_hir::def::Res::PrimTy(prim) => prim.to_api(cx),
...
impl<'ast, 'tcx> ToApi<'ast, 'tcx, TyKind<'ast>> for rustc_hir::QPath<'tcx> {
    fn to_api(&self, cx: &'ast RustcContext<'ast, 'tcx>) -> TyKind<'ast> {
        match self {
...
pub fn create_from_hir_ty<'ast, 'tcx>(
    ...
    let kind = match &rustc_ty.kind {
        ...
        rustc_hir::TyKind::Ptr(mut_ty) => {
            TyKind::RawPtr(create_from_hir_ty(cx, mut_ty.ty), mutability_from_hir(mut_ty.mutbl))
        },
        ...
        rustc_hir::TyKind::Path(qpath) => qpath.to_api(cx),
